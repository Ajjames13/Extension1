...
export const SettingsPage = () => {
  ...
  const [checklistBlocking, setChecklistBlocking] = useState(false);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const blockingSetting = await db.settings.get("checklistBlocking");
      
      if (isMounted) {
        setQuestions(template);
        setChecklistBlocking(blockingSetting?.value === "true");
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save checklist blocking setting
      const updatedAt = new Date().toISOString();
      await db.settings.put({
        key: "checklistBlocking",
        value: String(checklistBlocking),
        updatedAt
      });
  ...
  return (
    <section className="settings-page">
      ...
      <div className="settings-page__section">
        <h3>Checklist Configuration</h3>
        <label className="settings-page__toggle" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <input
            type="checkbox"
            checked={checklistBlocking}
            onChange={(e) => setChecklistBlocking(e.target.checked)}
          />
          Block new reflections until daily checklist is complete
        </label>
      </div>
